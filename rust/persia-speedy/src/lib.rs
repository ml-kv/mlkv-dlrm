...
mod tests {
    ...
    use super::{Context, Endianness, Readable, Reader, Writable, Writer};
    ...
    fn simple_read_from_stream_unbuffered() {
        ...
        let value =
            SimpleStruct::read_from_stream_unbuffered_with_ctx(Endianness::NATIVE, cursor).unwrap();
    ...
    fn simple_read_from_stream_buffered() {
        ...
        let cursor = io::Cursor::new(data);
        let value =
            SimpleStruct::read_from_stream_buffered_with_ctx(Endianness::NATIVE, cursor).unwrap();
        assert_eq!(value, SimpleStruct { a: 1, b: 2, c: 3 });
    }
    ...
    fn simple_read_bytes_from_buffer_owned() {
        ...
        let value: Cow<[u8]> =
            Readable::read_from_buffer_owned_with_ctx(Endianness::LittleEndian, &data).unwrap();
    ...
    fn simple_read_bytes_from_buffer_borrowed() {
        let data = vec![2, 0, 0, 0, 12, 34];
        let value: Cow<[u8]> =
            Readable::read_from_buffer_with_ctx(Endianness::LittleEndian, &data).unwrap();
        assert_eq!(&*value, &[12, 34]);
    ...
    fn write_to_stream_with_default_ctx() {
        let mut buffer = [0, 0];
        2_u16
            .write_to_stream(io::Cursor::new(&mut buffer[..]))
            .unwrap();
        assert_eq!(buffer, [2, 0]);
    }
    ...
    fn read_write_u8_vec() {
        ...
        let deserialized: Vec<u8> =
            Vec::<u8>::read_from_buffer_with_ctx(Endianness::NATIVE, &serialized).unwrap();
    ...
    fn read_write_u64_vec() {
        ...
        let deserialized: Vec<u64> =
            Vec::<u64>::read_from_buffer_with_ctx(Endianness::NATIVE, &serialized).unwrap();
        assert_eq!(original, deserialized);
    }
    ...
    fn read_write_string() {
        ...
        let deserialized: String =
            String::read_from_buffer_with_ctx(Endianness::NATIVE, &serialized).unwrap();
        assert_eq!(original, deserialized);
    }
    ...
    fn read_write_result() {
        ...
        let deserialized: Result<(), ()> =
            Readable::read_from_buffer_with_ctx(Endianness::NATIVE, &serialized).unwrap();
        assert_eq!(original, deserialized);
    ...
    fn read_big_vector_of_vectors_from_stream_buffered() {
        ...
        impl io::Read for TestStream {
            fn read(&mut self, output: &mut [u8]) -> Result<usize, io::Error> {
                ...
                let length =
                    std::cmp::min(random % output.len() + 1, self.buffer.len() - self.position);
                output[..length]
                    .copy_from_slice(&self.buffer[self.position..self.position + length]);
