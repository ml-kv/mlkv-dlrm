...
impl fmt::Display for Error {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        match self.kind {
            ...
            ErrorKind::ZeroNonZero => {
                write!(fmt, "a field which is supposed to be non-zero is zero")
            }
            ErrorKind::OutOfRangeLength => write!(fmt, "out of range length"),
            ErrorKind::OutOfRangeUsize => {
                write!(fmt, "value cannot fit into an usize on this architecture")
            }
            ErrorKind::UnexpectedEndOfInput => write!(fmt, "unexpected end of input"),
            ErrorKind::UnexpectedEndOfOutputBuffer => {
                write!(fmt, "unexpected end of output buffer")
            }
            ErrorKind::InputBufferIsTooSmall {
                ...
                fmt,
                "input buffer is too small; expected at least {} bytes, got {}",
            ...
            ErrorKind::LengthIsNotTheSameAsLengthAttribute { field_name } => write!(
                fmt,
                "the length of '{}' is not the same as its 'length' attribute",
                field_name
            ),
            ErrorKind::ExpectedConstant { constant } => write!(
                fmt,
                "expected a predefined {} bytes(s) long constant",
                constant.len()
            ),
...
impl IsEof for Error {
    fn is_eof(&self) -> bool {
        match self.kind {
            ErrorKind::UnexpectedEndOfInput | ErrorKind::UnexpectedEndOfOutputBuffer => true,
...
#[cold]
pub fn error_invalid_string_utf8<T>(_: std::string::FromUtf8Error) -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_invalid_str_utf8<T>(_: std::str::Utf8Error) -> T
where
    T: From<Error>,
{
    T::from(Error::new(ErrorKind::InvalidUtf8))
...
#[cold]
pub fn error_length_is_not_the_same_as_length_attribute<T>(field_name: &'static str) -> T
where
    T: From<Error>,
{
    T::from(Error::new(ErrorKind::LengthIsNotTheSameAsLengthAttribute {
        field_name,
    }))
}

#[cold]
pub fn error_out_of_range_length<T>() -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_invalid_enum_variant<T>() -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_out_of_range_char<T>() -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_too_big_usize_for_this_architecture<T>() -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_end_of_output_buffer<T>() -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_input_buffer_is_too_small<T>(actual_size: usize, expected_size: usize) -> T
where
    T: From<Error>,
{
    T::from(Error::new(ErrorKind::InputBufferIsTooSmall {
        actual_size,
        expected_size,
    }))
}
...
#[cold]
pub fn error_output_buffer_is_too_small<T>(actual_size: usize, expected_size: usize) -> T
where
    T: From<Error>,
{
    T::from(Error::new(ErrorKind::OutputBufferIsTooSmall {
        actual_size,
        expected_size,
    }))
}
...
#[cold]
pub fn error_zero_non_zero<T>() -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_invalid_system_time<T>() -> T
where
    T: From<Error>,
{
...
#[cold]
pub fn error_expected_constant<T>(constant: &'static [u8]) -> T
where
    T: From<Error>,
{
