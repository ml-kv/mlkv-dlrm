use {
    crate::{
        error::{error_expected_constant, error_invalid_str_utf8, error_invalid_string_utf8},
        Context, Error, Readable, Reader, Writable, Writer,
    },
    std::borrow::Cow,
};

pub use crate::error::{
    error_invalid_enum_variant, error_length_is_not_the_same_as_length_attribute,
    error_out_of_range_length, get_error_kind, ErrorKind,
};
pub use crate::varint::VarInt64;

#[inline]
pub fn vec_to_string<E>(bytes: Vec<u8>) -> Result<String, E>
where
    E: From<Error>,
{
...
#[inline]
pub fn cow_bytes_to_cow_str<E>(bytes: Cow<[u8]>) -> Result<Cow<str>, E>
where
    E: From<Error>,
{
    match bytes {
...
impl<'a, T> IntoLength for &'a mut T
where
    T: IntoLength + Copy,
{
    fn into_length(self) -> usize {
...
where
    ...
    R: Reader<'a, C>,
{
    let is_ok = if let Some(result) = reader.read_bytes_borrowed(constant.len()) {
        result? == constant
    } else {
